...
RAD = 10#20 # Radius of node circles on the screen, in pixels. Also used elsewhere,
...
def show_edge(x1,y1,x2,y2,starting_level,state1_string,state2_string,label='',dx=0,dy=0,color='gray',rad=10):
...
def start_HMM_display(S, expected_N, width=800, height=500,
...
def show_entire_trellis(S, observation_sequence, width=1500, height=800, has_initial_state=False):
...
def show_label_at_node(layer_no, state_string, text, dx=0, dy=0, color='navy'):
...
def show_label_at_edge(layer_no, tail_state_string, head_state_string, text, dx=0, dy=0, color='navy'):
...
def highlight_edge(layer_no, state_string1, state_string2, highlight=True):
    ...
    if highlight:
        DIAGRAM.itemconfig(edge, width=4, fill='red')
    else:
